...
/**
 * A particle with pos, velocity, and force vectors and mass
 */
...
Particle.prototype.distanceTo = function(p) { return this.pos.distanceTo(p.pos); };
...
RungeKuttaIntegrator.prototype.allocateParticles = function() {
	while (this.s.particles.length > this.originalPoss.length) {
		this.originalPoss.push(new Vector());
...
RungeKuttaIntegrator.prototype.step = function (deltaT) {
	...
	for (i = 0; i < s.particles.length; i++) {
		p = s.particles[i];
		if (!p.fixed) {
			originalPos = this.originalPoss[i];
			k1Velocity = this.k1Velocities[i];
			p.pos.x = originalPos.x + k1Velocity.x * 0.5 * deltaT;
			p.pos.y = originalPos.y + k1Velocity.y * 0.5 * deltaT;
			p.pos.z = originalPos.z + k1Velocity.z * 0.5 * deltaT;
	...
	for (i = 0; i < s.particles.length; i++) {
		...
		if (!p.fixed) {
			...
			p.pos.x = originalPos.x + k2Velocity.x * 0.5 * deltaT;
			p.pos.y = originalPos.y + k2Velocity.y * 0.5 * deltaT;
			p.pos.z = originalPos.z + k2Velocity.z * 0.5 * deltaT;
			originalVelocity = this.originalVelocities[i];
	...
	for (i = 0; i < s.particles.length; i++) {
		...
		if (!p.fixed) {
			...
			p.pos.x = originalPos.x + k3Velocity.x * deltaT;
			p.pos.y = originalPos.y + k3Velocity.y * deltaT;
			p.pos.z = originalPos.z + k3Velocity.z * deltaT;
			originalVelocity = this.originalVelocities[i];
	...
	for (i = 0; i < s.particles.length; i++) {
		...
		p.pos.x = originalPos.x + deltaT / 6.0 * (k1Velocity.x + 2.0*k2Velocity.x + 2.0*k3Velocity.x + k4Velocity.x);
		p.pos.y = originalPos.y + deltaT / 6.0 * (k1Velocity.y + 2.0*k2Velocity.y + 2.0*k3Velocity.y + k4Velocity.y);
		p.pos.z = originalPos.z + deltaT / 6.0 * (k1Velocity.z + 2.0*k2Velocity.z + 2.0*k3Velocity.z + k4Velocity.z);
